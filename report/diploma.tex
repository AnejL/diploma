...
\noindent Strežnik ima tri naloge:
\begin{enumerate}
...
API uporabniku omogoča registracijo in prijavo.
Za registracijo in prijavo sta potrebna geslo in uporabniško ime (e-mail).
Ko se uporabnik registrira, lahko ustvarja svoje opise tehnoloških postopkov.

Ko ustvari nov opis tehnološkega postopka (v nadaljevanju \textit{projekt}), ta projekt dobi status "odprt" (\textit{ang. opened}).
Ob zaključenem delu ali odprtju novega projekta se status "odprt" odstrani.
V naši implementaciji smo to naredili z stolpcem "Opened" v tabeli Projects, ki hrani bit 0 ali 1.
...
\begin{tabular}{|c | c | c|} 
 \hline
 URL & Metoda & Rezultat \\ [0.5ex] 
 \hline
 /identity/login & POST & prijava uporabnika \\ 
 \hline
 /identity/register & POST & registracija uporabnika \\ 
 \hline
 /projects & GET & vsi projekti prijavljenega uporabnika \\ 
 \hline
 /projects/opened & GET & odprt projekt prijavljenega uporabnika \\ 
 \hline
 /register & POST & registracija uporabnika \\ [1ex]
 \hline
\end{tabular}

\subsection{Implementacija delavca}
